...
def test_submit_essay():
    url = 'http://127.0.0.1:2003/submit_essay'  # Nota el cambio en el puerto a 2003
    ...
    if response.status_code == 200:
        ...
        for line in response.iter_lines():
            if line:
                # Decodificar la línea y eliminar el prefijo 'data: '
                line = line.decode('utf-8')
                if line.startswith('data: '):
                    try:
                        # Parsear el JSON después de 'data: '
...
def test_ask_questions():
    ...
    data = {
        'student_questions': [
            "¿podrías repetir Qué puedo mejorar en la argumentacion?"
        ]
    }
    ...
    if response.status_code == 200:
        print("Recibiendo respuesta a las preguntas:")
        for line in response.iter_lines():
            if line:
                line = line.decode('utf-8')
                if line.startswith('data: '):
...
def test_submit_criteria():
    url = 'http://127.0.0.1:2003/submit_criteria'
    ...
    if response.status_code == 200:
        print("Criterios enviados exitosamente:")
        print(response.json())
    else:
        print(f"Error {response.status_code}: {response.text}")
...
if __name__ == "__main__":
    # Primero enviamos los criterios
    print("=== Enviando criterios ===")
    test_submit_criteria()

    # Luego enviamos el ensayo
    print("\n=== Enviando ensayo ===")
    test_submit_essay()

    # Finalmente hacemos algunas preguntas
    print("\n=== Enviando preguntas ===")
    test_ask_questions()