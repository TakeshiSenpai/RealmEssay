...
async function run() {
  try {
    ...
    const activeTeachers = await collection.find({ Estado: 'Activo' }).toArray();
    console.log('Profesores activos:', JSON.stringify(activeTeachers, null, 2));

    // Ejemplo: obtener un solo documento donde el campo 'Nombre' sea 'Dr. Juan Pérez'
    const singleDoc = await collection.findOne({ Nombre: 'Dr. Juan Pérez' });
