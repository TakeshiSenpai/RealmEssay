...
async function run() {
  try {
    ...
    const activeTeachers = await collection.find({ date: '2024-09-28' }).toArray();
    console.log('Tareas actuales:', JSON.stringify(activeTeachers, null, 2));

    // Ejemplo: obtener un solo documento donde el campo 'Nombre' sea 'Dr. Juan Pérez'
    const singleDoc = await collection.findOne({ author: 'Alan' });
